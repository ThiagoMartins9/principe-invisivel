...
export function buildMissionEl(m, opts = {}){
  ...
  const handleHTML = opts.draggable
    ? `<div class="drag-handle" data-grip="${m.id}" title="Arrastar para reordenar (ou setas ↑↓)" aria-label="Arrastar para reordenar" role="button" tabindex="0">
        <svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor" aria-hidden="true"><circle cx="9" cy="5" r="1.7"/><circle cx="15" cy="5" r="1.7"/><circle cx="9" cy="12" r="1.7"/><circle cx="15" cy="12" r="1.7"/><circle cx="9" cy="19" r="1.7"/><circle cx="15" cy="19" r="1.7"/></svg>
      </div>`
    : "";
  el.innerHTML = `
    ${handleHTML}
...
export function renderMissions(){
  ...
  // Arrasto só faz sentido quando a lista mostra a sequência manual completa:
  // sem busca e sem filtro de tag (posições não-contíguas seriam ambíguas).
  // Concluídas seguem cronológicas (doneAt) e não têm alça.
  const canDrag = !q && !ui.activeTagFilter && ui.listFilter !== "done";
  arr.forEach(m => list.appendChild(buildMissionEl(m, { draggable: canDrag && !m.doneAt })));
}

/**
 * P6 — ordenação da lista por categoria.
 * Pendentes: ordem manual (m.order asc); concluídas ao fim, por doneAt desc.
 * Fallback para missões sem order (estado pré-migração vindo de sync): fim da
 * lista, desempate por createdAt desc.
 */
function missionListCmp(a, b){
  if(!a.doneAt && !b.doneAt){
    const ao = (typeof a.order === "number" && isFinite(a.order)) ? a.order : Infinity;
    const bo = (typeof b.order === "number" && isFinite(b.order)) ? b.order : Infinity;
